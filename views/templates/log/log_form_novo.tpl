                ...
                <form name="frm-log" action="/log/gravar_log" method="POST" enctype="multipart/form-data" onsubmit="return validaFormulario()" role="form">
                    <div class="row">
                        <div class="form-group  col-lg-6">
                            ...
                            {if {$registro.id_log}>0}                        
                                <input class="form-control" id="disabledInput" type="text" value="{$registro.id_log}" disabled>
                            {else}
                                <input class="form-control" id="disabledInput" type="text" value="Novo Registro" disabled>
                            {/if}
                        </div>          
                        <input type="hidden" name="id_log" value="{$registro.id_log}" />
                        <div class="form-group col-lg-6">
                            <label  class="control-label" for="id_usuario">Usuário</label>
                            {html_options name=id_usuario options=$lista_usuarios class="form-control"}
                        </div>
                    </div>

                    <div class="form-group">
                        <label class="control-label" for="id_log_tipo">Tipo</label>
                        {html_options name=id_log_tipo options=$lista_tipos_log class="form-control" id=id_log_tipo}
                    </div>
                    <div class="form-group">
